...
./approxmc/approxmc ${cnf_file}
if [ $? -ne 0 ]; then
    echo "Error: approxmc failed for $model_name"
    exit 1
fi

END_TIME=$(date +%s)
ELAPSED_TIME=$((END_TIME - START_TIME))
echo "approxmc execution time for $model_name on feature $feature : $ELAPSED_TIME seconds"
