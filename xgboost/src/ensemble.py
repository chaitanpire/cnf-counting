...
class Ensemble:
    ...
    def load( self, model_file=None, *arg ):
        ...
        # --------------------------------------------------------------
        if self.feature_names == None: self.feature_names = feature_names
        ...
        if self.options.multiclass:
            if self.n_classes < 3:
                print("Less than three output classes! Switiching to binary.")
                self.options.multiclass = False
        else:
            if self.n_classes != 1:
                print("Model is not binary classifier! Switiching to multiclass analysis!")
